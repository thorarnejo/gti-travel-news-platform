...
import { MapPin, Clock, Plane, Building, Map, FileText, Shield, Cloud, Zap, AlertCircle } from 'lucide-react'
...
function getActionLine(whatToDo: string | null | undefined): string {
  if (!whatToDo) return 'No action needed'
  ...
  return firstLine
    .replace(/^\d+\.\s*/, '')
    .replace(/^-\s*/, '')
    .replace(/^(Check|Verify|Book|Expect|Consider|Avoid|Arrive|Depart)/i, match => match)
    .trim()
    .slice(0, 60) + (firstLine.length > 60 ? '...' : '')
}
...
export function ArticleCard({ article, variant = 'default', className }: ArticleCardProps) {
  ...
  if (variant === 'featured') {
    return (
      ...
      >
        ...
        <h2 className={cn("text-xl font-bold mb-2 leading-tight", isCritical && "text-red-900")}>{article.headline}</h2>
  ...
  return (
    ...
    >
      ...
      <h3 className={cn("font-semibold text-base mb-2 leading-snug line-clamp-2", isCritical && "text-red-800")}>
        {article.headline}
      </h3>
